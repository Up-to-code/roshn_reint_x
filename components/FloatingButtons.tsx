...
export function FloatingButtons({ locale }: FloatingButtonsProps) {
  ...
  return (
    <div
      className={`fixed bottom-6 z-50 flex ${
        isRTL ? "right-6" : "left-6"
      }`}
    >
      <button
        onClick={handleWhatsApp}
        className="flex items-center gap-3 rounded-full bg-[#25D366] px-6 py-4 text-white shadow-xl transition-all duration-300 hover:scale-110 hover:bg-[#128C7E]"
